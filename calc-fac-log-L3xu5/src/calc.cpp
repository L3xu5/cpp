...
std::size_t constexpr max_decimal_digits = 10;
...
double factorial(double current)
{
    double result = 1;
    while (current > 1.) {
        result *= current--;
...
double unary(const double current, const Op op)
{
    ...
    case Op::FAC: {
        double integer_part;
        modf(current, &integer_part);
        if (current < 0 || current - integer_part >= 1e-13) {
            std::cerr << "Bad argument for factorial: " << current << std::endl;
            return current;
        }
        return factorial(integer_part);
    }
    default:
        return current;
    }
